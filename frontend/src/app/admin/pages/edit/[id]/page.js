...
import {
  ArrowLeft, Plus, Trash2, ArrowUp, ArrowDown, Loader2, Layout, Type,
  Image as ImageIcon, GalleryHorizontal, MousePointerClick,
} from 'lucide-react';
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
          ...
          <span className={`w-1.5 h-1.5 rounded-full ${status === 'active' ? 'bg-green-500' : 'bg-amber-500'}`} />
                ...
                <div className="px-5 pb-5 space-y-3">
                  ...
                  {section.type === 'cta' && (
                    <>
                      ...
                      <input placeholder="Button Link" value={section.data.buttonLink || ''} onChange={(e) => updateSectionField(index, 'buttonLink', e.target.value)} className={fieldClass} />
                    </>
                  )}
                  {section.type === 'text' && (
                    <>
      ...
      <div className="flex gap-3 pb-8">
        <button disabled={saving} onClick={() => saveChanges('draft')} className="px-5 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-semibold text-sm hover:bg-slate-200 transition disabled:opacity-50">
          Save as Draft
        </button>
        <button disabled={saving} onClick={() => saveChanges('active')} className="px-5 py-2.5 rounded-xl text-white font-semibold text-sm shadow-md hover:opacity-90 transition disabled:opacity-50" style={{ background: 'var(--pa-primary,#2f9e44)' }}>
